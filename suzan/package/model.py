...
class RadiationModel(Model):
    ...
    def set_grid_amplitudes(self, n_or_dd: int, random: bool = True,
        ...
        if fix_params:
            ...
            self.set_hyperparams(self.azimuth, self.takeoff_angles,
                                 self.velocities)
            
        else:
            params  = None
            if random:
                params = fn.random_params(n_or_dd)
            else: 
                params = fn.systematic_params(n_or_dd)
            
            assert params is not None, "sdr parameters needed for forward simulation"
    ...
    #
    def filter_outliers(self, z_threshold=2):
