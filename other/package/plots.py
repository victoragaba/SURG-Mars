...
def iterates_2D(model: sm.RadiationModel, cmap='rainbow', s=10, optimal=True,
    ...
    if uncertainty: # TODO
        ...
        print(f"Covariances shape: {len(covariances)}")
...
def amplitudes(model: sm.RadiationModel, elev=30, azim=45, cmap='rainbow', s=50, alpha=0.5,
               ...
               eps_factor=1e-2, optimal=True, uncertainty=False, conf=0.95, grid_mask=[1, 1, 1]):
    '''
    ...
    if grid:
        ...
        grid_colors = np.array(model.get_grid_colors())
        mins = grid_colors.min(axis=0)
        maxs = grid_colors.max(axis=0)
        grid_colors = (grid_colors - mins) / (maxs - mins + 1e-8)
        # block the channels using grid_mask
        grid_colors = grid_colors * np.array(grid_mask)

        # plot the scatter graph
        ax.scatter(
            grid_AP, grid_ASV, grid_ASH,
            c=grid_colors, marker='o', s=0.01*s, alpha=0.5*alpha, label='Grid points'
        )
    ...
    if cross_section != [0,0,0]:
        if grid:
            ...
            range_ASV = max_ASV - min_ASV
            range_ASH = max_ASH - min_ASH
            ...
            for idx, x in enumerate(x_sections):
                ...
                if grid:
                    ...
                    axes[idx].scatter(
                        ...
                        c=[grid_colors[i,:] for i in x_grid_points],
                        marker='o', s=0.1*s, alpha=0.5*alpha, label='Grid points'
                    )
                    # adjust the axes to match the global scales
                    axes[idx].set_xlim(min_ASV - padding*range_ASV, max_ASV + padding*range_ASV)
                    axes[idx].set_ylim(min_ASH - padding*range_ASH, max_ASH + padding*range_ASH)
            ...
            for idx, y in enumerate(y_sections):
                ...
                if grid:
                    ...
                    axes[idx].scatter(
                        ...
                        c=[grid_colors[i,:] for i in y_grid_points],
                        marker='o', s=0.1*s, alpha=0.5*alpha, label='Grid points'
                    )
                    # adjust the axes to match the global scales
                    axes[idx].set_xlim(min_AP - padding*range_AP, max_AP + padding*range_AP)
                    axes[idx].set_ylim(min_ASH - padding*range_ASH, max_ASH + padding*range_ASH)
                if iterates:
            ...
            for idx, z in enumerate(z_sections):
                ...
                if grid:
                    ...
                    axes[idx].scatter(
                        ...
                        c=[grid_colors[i,:] for i in z_grid_points],
                        marker='o', s=0.1*s, alpha=0.5*alpha, label='Grid points'
                    )
                    # adjust the axes to match the global scales
                    axes[idx].set_xlim(min_AP - padding*range_AP, max_AP + padding*range_AP)
                    axes[idx].set_ylim(min_ASV - padding*range_ASV, max_ASV + padding*range_ASV)
                if iterates:
...
def beachballs(model: sm.RadiationModel, order_by='strike',width=10, max_plot=50,
    ...
    if original:  # only when synthetic data is used
        ...
        if order_by == 'tensor':
            ...
            Ao_mt = fn.enu2use(Ao_mt) # TODO: is this necessary?
            Ao_comp_mt = np.array([
    ...
    assert order_by in ['strike', 'dip', 'rake', 'tensor', 'kagan'], f'Invalid order_by value'
    if order_by == 'tensor':
        ...
        elif order_by == 'kagan': order = np.arange(len(solution_set))
