...
def iterates_2D(model: sm.RadiationModel, cmap='rainbow', s=10, optimal=True, index=2):
    '''
...
def tp_axes(model: sm.RadiationModel, elev=30, azim=45, half=False, central=False):
    '''
    Make a 3D plot of the optimal tp axes.
    
    Args:
        central (bool): If True, plot the central axes instead of optimal axes.
    '''
    # BUG? check it out and make sure
...
def beachballs(model: sm.RadiationModel, central=False, order_by='strike',width=10,
               max_plot=50, facecolor='blue'):
    '''
    Plot beachballs for the optimal solutions.
    central: if True, plot central mechanisms instead of optimal mechanisms.
    ...
    '''
    assert order_by in ['strike', 'dip', 'rake'], f'Invalid order_by value'
