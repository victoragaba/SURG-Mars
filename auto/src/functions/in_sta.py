...
def read_reverse_skhash_file(plfile,merge_on):
	...
	pol_reverse_df['start_time']=pd.to_datetime(pol_reverse_df['start_time'],errors = 'coerce')
	pol_reverse_df['end_time']=pd.to_datetime(pol_reverse_df['end_time'],errors = 'coerce')
	if pd.isnull(pol_reverse_df['start_time']).any():
		raise ValueError('Error with reading start times from polarity reversal file (plfile: {})')
		# pol_reverse_df['start_time'].fillna(pd.Timestamp('1900-01-01'))
	if pd.isnull(pol_reverse_df['end_time']).any():
		raise ValueError('Error with reading end times from polarity reversal file (plfile: {})')
		# pol_reverse_df['end_time'].fillna(pd.Timestamp('2200-01-01'))
