...
class RadiationModel(Model):
    ...
    def is_kagan_sorted(self):
        '''
        Return whether the optimal iterates are sorted by kagan angle.
        '''
        return self.kagan_sorted
    ...
    def set_real_sdr(self, sdr):
        ''' Set the real sdr for known mechanism. '''
        self.real_sdr = sdr
    
    
    def get_real_sdr(self):
        ''' Get the real sdr for known mechanism. '''
        return self.real_sdr
    ...
    def __call__(self, params, set_Ao=False, return_A=False):
        ...
        if set_Ao:
            self.Ao_params = params
            if self.hidden:
                self.Ao_tp = fn.hidden2tp(params, az=self.phi, in_deg=False)
            else:
                self.set_real_sdr(params)
                self.Ao_tp = fn.sdr2tp(params)
            self.set_Ao(self.A)
    ...
    def set_grid_amplitudes(self, n_or_dd: int, random: bool = True,
                            fix_params: bool = False, zne: bool = False,
                            extract_range: list = [], ps_gap: float = 0.):
        '''
        Args:
        ...
        if zne:
            # this needs dd, not n
            thetas = np.arange(0, 360, n_or_dd)
            Z_axis = fn.k_hat
            N_axis = fn.j_hat
            E_axis = fn.i_hat
            
            # first set up the original t and p axes, in self.Ao_tp
            Ao_t, Ao_p = self.get_Ao_tp()
            ...
            for theta in thetas:
                tZ = fn.rotate_vec(Ao_t, Z_axis, np.deg2rad(theta))
                tN = fn.rotate_vec(Ao_t, N_axis, np.deg2rad(theta))
                tE = fn.rotate_vec(Ao_t, E_axis, np.deg2rad(theta))
                pZ = fn.rotate_vec(Ao_p, Z_axis, np.deg2rad(theta))
                pN = fn.rotate_vec(Ao_p, N_axis, np.deg2rad(theta))
                pE = fn.rotate_vec(Ao_p, E_axis, np.deg2rad(theta))
                Z_sdr = fn.tp2sdr(tZ, pZ)[0]
                N_sdr = fn.tp2sdr(tN, pN)[0]
                E_sdr = fn.tp2sdr(tE, pE)[0]
                self.grid_amplitudes.append(self(Z_sdr, return_A=True))
                self.grid_colors.append([theta, 0, 0]) # TODO
                self.grid_amplitudes.append(self(N_sdr, return_A=True))
                self.grid_colors.append([0, theta, 0]) # TODO
                self.grid_amplitudes.append(self(E_sdr, return_A=True))
                self.grid_colors.append([0, 0, theta]) # TODO
        ...
        if fix_params:
            ...
            for param in params:
                _ = self(param)
                self.grid_amplitudes.append(self.A)
                self.grid_colors.append(param) # TODO
    ...
    def get_grid_colors(self):
        ''' Return the colors from grid search (for manifold). '''
    ...
    def get_optimal_iterates(self):
        ...
        Kagan ref is sdr of main beachball
        '''
        ...
        # order by kagan angle relative to real sdr, must be available
        # print("NEW CODE")
        if not self.is_kagan_sorted():
            # print("Sorting optimal iterates by kagan angle...")
            angles = [fn.sdr2kagan(self.optimal_iterates[i], self.real_sdr, deg=False)
                        for i in range(len(self.optimal_iterates))]
            order = np.argsort(angles)
            self.optimal_iterates = [self.optimal_iterates[i] for i in order]
            if self.hidden:
                self.hidden_optimal_iterates = [self.hidden_optimal_iterates[i]
                                                for i in order]
            self.kagan_angles = [angles[i] for i in order]
            self.kagan_sorted = True
            

        return self.optimal_iterates
    
    
    def get_kagan_angles(self):
        ''' Return the kagan angles of the optimal iterates. '''
        if not self.is_kagan_sorted():
            _ = self.get_optimal_iterates()
        return self.kagan_angles
