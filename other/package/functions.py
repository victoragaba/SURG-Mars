...
def enu2ned(M: np.array) -> np.array:
    '''
    Convert moment tensor from ENU to NED system
    '''
    enu_ned_P = np.array([[0, 1, 0],
                          [1, 0, 0],
                          [0, 0, -1]])
    
    return enu_ned_P @ M @ enu_ned_P.T
...
def zne2zrt_matrix(baz: float, deg: bool = True) -> np.array:
    '''
    Returns the rotation matrix to convert from ZNE to ZRT coordinate system.

    Args:
        baz (float): station back azimuth in RADIANS
    '''
    if deg:
        baz = np.deg2rad(baz)
...
def zrt2lqt_matrix(p_inc: float = None, s_inc: float = None, deg: bool = True) -> np.array:
    '''
    Returns the rotation matrix to convert from ZRT to LQT coordinate system.

    Args:
        p_inc (float, optional): P wave incidence angle in RADIANS. Defaults to None.
        s_inc (float, optional): S wave incidence angle in RADIANS. Defaults to None.
    '''
    if p_inc is None and s_inc is None:
        raise ValueError("At least one of p_inc or s_inc must be provided.")
    if p_inc is None: p_inc = s_inc
    if s_inc is None: s_inc = p_inc
    if deg:
        p_inc = np.deg2rad(p_inc)
        s_inc = np.deg2rad(s_inc)

    R = np.array([[np.cos(p_inc), np.sin(p_inc), 0],
                    [-np.sin(s_inc), np.cos(s_inc), 0],
                    [0, 0, 1]])
    
    return R


def sdr2kagan(sdr1: list, sdr2: list, deg: bool = True) -> float:
    '''
    Returns the Kagan angle between two focal mechanisms given their SDR parameters.
    Input and output are in specified units.
    
    Args:
        sdr1 (list): first mechanism's SDR parameters in 
        sdr2 (list): second mechanism's SDR parameters in RADIANS
    '''
    t1, p1 = sdr2tp(sdr1, deg=deg)
    t2, p2 = sdr2tp(sdr2, deg=deg)
    return tp2kagan(t1, p1, t2, p2, deg=deg)


def mt2kagan(mt1: np.array, mt2: np.array, deg: bool = True) -> float:
    '''
    Returns the Kagan angle between two focal mechanisms given their moment tensors.
    Output is in specified units.
    
    Args:
        mt1 (np.array): first mechanism's moment tensor
        mt2 (np.array): second mechanism's moment tensor
    '''
    t1, p1 = mt2tp(mt1)
    t2, p2 = mt2tp(mt2)
    return tp2kagan(t1, p1, t2, p2, deg=deg)
...
def tp2kagan(t1: list, p1: list, t2: list, p2: list, deg: bool = True) -> float:
    '''
    Returns the Kagan angle between two focal mechanisms given their T and P axes.
    Output is in specified units.
    
    Args:
        t1 (list): first mechanism's T axis
        p1 (list): first mechanism's P axis
        t2 (list): second mechanism's T axis
        p2 (list): second mechanism's P axis
    '''
    b1 = -unit_vec(np.cross(t1, p1))
    b2 = -unit_vec(np.cross(t2, p2))
    
    q1 = npq.from_rotation_matrix(np.column_stack((t1, b1, p1)))
    q2 = npq.from_rotation_matrix(np.column_stack((t2, b2, p2)))
    
    q_rel = npq.as_float_array(q1.conjugate() * q2)
    max_abs = np.max(np.abs(q_rel))
    angle = 2*np.arccos(max_abs)
    if deg: angle = np.rad2deg(angle)
    return angle


def aic(data: np.array) -> np.array:
    """
