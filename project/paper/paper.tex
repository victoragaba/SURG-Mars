...
% \title{Single-seismometer Focal Mechanism and Uncertainty Estimation }
\title{An Asymmetry-Aware Method for Single-seismometer Focal Mechanism Inversion and Uncertainty Quantification}
\shorttitle{Single-seismometer Focal Mechanism Inversion and Uncertainty Quantification from Body Waves} % used for header, not mandatory
  ...
  \subsection{Misfit gradient}
    ...
    \begin{figure}[ht!]
      ...
      \begin{tikzpicture}[every node/.style={circle, draw, minimum size=.8cm}]
        ...
        \draw[->] (H5) -- (A3);
      \end{tikzpicture}
      \caption{Computation graph of forward model (edges are non-zero partial derivatives).}
      \label{fig:1}
    \end{figure}
    ...
    \begin{align*}
      ...
      &g = \sin(\psi_r)\;\;\;\;\;\;\;\;\;\;\; h = \cos(\psi_r)\;\;\;\;\;\;\;\;\;\; p = cos(2(\psi_r)\\
      &q = \sin(2\psi_r)
    \end{align*}
    \begin{align} \label{eq:18}
      \nonumber&\bm{J_m}(\bm{\eta}) = \\
      &\begin{bmatrix}
        0         &ab         &0.5cd\\
    ...
    \begin{align} \label{eq:21}
      \nabla_{\bm{m}}\Phi = \bm{J_m}(\bm{\eta})^\top \bm{J_\eta}(\bm{A})^\top \nabla_{\bm{A}}\tilde{\Phi}
    \end{align}
