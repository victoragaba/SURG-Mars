...
# NOTE: This is a good place to poke...
...
# NOTE: manipulate this to extract travel times
def create_takeoff_table(vmodel_depthvp,deptab,delttab,nump,nx0,nd0,takeoff_az_precision):
	...
	time_table=np.zeros((nx0,len(deptab)))-999
	...
	for idep in range(ndep):
		...
		for idel in range(1,ndel):
			...
			t1_min = np.max(t1)
			min_ind=ind[np.argmax(t1)]

			scr1[idel]=psave[min_ind]/usave[min_ind]
			times2save[idel]=t1_min
	...
	# # save table to csv
	table_df=pd.DataFrame(table,columns=deptab,index=delttab)
	table_df.to_csv(os.path.join(os.path.dirname(__file__),'takeoff_table.csv'))
	# # save time table to csv
	# time_table_df=pd.DataFrame(time_table,columns=deptab,index=delttab)
	# time_table_df.to_csv(os.path.join(os.path.dirname(__file__),'maxtime_table.csv'))
...
# TODO: check this against kagan angle, find out what's up
...
# TODO: how is this grid search different from Maddy's?
...
# TODO: why do I keep running into mech quality issues?
